    ...
    <div className="min-h-screen pb-24 px-4 pt-4 max-w-lg mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="text-center mb-6">
          <span className="text-5xl block mb-3">{sp.icon}</span>
          <h1 className="font-display text-2xl font-bold">{sp.title}</h1>
          ...
          {sp.modules.map((mod, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                {mod.isBoss ? (
                  <div className="w-8 h-8 rounded-xl bg-streak/20 flex items-center justify-center">
                    <Swords size={16} className="text-streak" />
                  </div>
                ) : (
